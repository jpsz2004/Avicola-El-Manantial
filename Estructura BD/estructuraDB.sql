...
BEGIN
    ...
    WHERE producto_id = p_producto_id; -- Ahora referencia el parámetro de entrada
END //
...
BEGIN
    ...
    SET saldo = (SELECT total_venta FROM Ventas WHERE venta_id = venta_id_param) - monto_pagado;

    -- Insertar el pago
    INSERT INTO Pagos (venta_id, monto_pagado, fecha_pago, metodo_pago, estado, saldo_pendiente)
    VALUES (venta_id_param, monto_pagado, fecha_pago, metodo_pago, estado, saldo);
END //
